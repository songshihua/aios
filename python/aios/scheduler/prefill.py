...
class PrefillManager:
    ...
    def _can_admit(
        ...
        # Running requests keep their table slots, so table availability is
        # also the max-running admission limit, as in mini-sglang.
    ...
    def schedule_next_batch(self, prefill_budget: int) -> Batch | None:
        ...
        for pending in self.pending_list:
            ...
            if selected and scheduled_tokens + prompt_len > prefill_budget:
                break
            if prompt_len > prefill_budget and selected:
                break
            if not self._can_admit(
                pending, len(selected), scheduled_reserved
            ):
                break
            selected.append(pending)
            scheduled_tokens += prompt_len
            scheduled_reserved += prompt_len + pending.output_len
            if scheduled_tokens >= prefill_budget:
                break
